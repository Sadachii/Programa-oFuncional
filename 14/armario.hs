...
consulta indice armarios = consulta' indice $ Map.toList armarios
    where 
        consulta' index [] = Nothing
        consulta' index ((int, armario):xs)
            | index == int = Just (fst armario)
            | otherwise = consulta' index xs


-- dado o número do armário, retorna Right armário ou Left "Armario nao existe"
pegarArmario :: Int -> Armarios -> Either String Armario
pegarArmario indice armarios = pegar indice $ Map.toList armarios
    where 
        pegar index [] = Left ("Armario " ++ show indice ++ " nao existe")
        pegar index ((int, armario):xs)
            | index == int = Right armario
            | otherwise = pegar index xs
...
aluguel indice armarios = case pegarCodigo indice armarios of
                            Left x -> Left x
                            Right x -> Right $ Map.insert indice (Alugado, x) armarios
...
devolucao indice codigo armarios = case armario of
                                    Left x -> Left x 
                                    Right x -> funcao x
    where 
        funcao x
            | fst x /= Alugado = Left ("Armario " ++ show indice ++ " desocupado")
            | snd x /= codigo = Left "Codigo incorreto"
            | otherwise = Right $ Map.insert indice (Livre, codigo) armarios
